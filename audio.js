// audio.js — Music Player + Visualizer (يعتمد على config.js فقط)
(() => {
  const CFG = window.APP_CONFIG;
  if (!CFG || !CFG.musicPlayer) return console.error("APP_CONFIG.musicPlayer missing");
  ...
  if (!audioEl || !btn || !vol || !titleEl || !prevBtn || !nextBtn || !canvas) {
    return console.error("Audio UI elements missing");
  }
  ...
  function drawBars(){
    const w = canvas.getBoundingClientRect().width;
    const h = canvas.getBoundingClientById ? 0 : canvas.getBoundingClientRect().height; // safe
    const hh = canvas.getBoundingClientRect().height;

    ctx2d.clearRect(0, 0, w, hh);
    ctx2d.fillStyle = "rgba(255,255,255,0.03)";
    ctx2d.fillRect(0, 0, w, hh);
